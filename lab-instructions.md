...
The _BudgetEase_ conference expense planner features will include:
...
> _Replace `<repository-link>` with the link to your forked `conference_event_planner` repository._
...
_Note: While pushing the code to your GitHub repository, it might ask you to provide a username and password. Review [these instructions](https://cf-courses-data.static.labs.skills.network/IBMSkillsNetwork-CD0210EN-Coursera/labs/v2/m1/Setting_up_the_environment.md.html) if you need more guidance on how to work in GitHub._
...
_Note: Include above code after `const venueItems = useSelector((state) => state.venue);` `ConferenceEvent.jsx` component._
...
- Each row of the table displays the following information, respectively
  ...
  - For meals, it uses the `numberOfPeople` property to display \"For _x_ people\" where _x_ is the number of people.
